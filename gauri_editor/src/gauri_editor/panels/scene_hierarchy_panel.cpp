...
void SceneHierarchyPanel::OnImGuiRender()
{
    ...
    if (ImGui::IsMouseDown(ImGuiMouseButton_Left) && ImGui::IsWindowHovered(ImGuiHoveredFlags_None))
    {
        m_SelectionContext = {};
    }
...
void SceneHierarchyPanel::DrawComponents(Entity entity)
{
    ...
    if (entity.HasComponent<TransformComponent>())
    {
        ...
        if (open)
        {
            ...
            DrawVec3Control("Translation", tc.Translation, 0.1f);
            glm::vec3 rotation = glm::degrees(tc.Rotation);
            DrawVec3Control("Rotation", rotation, 0.1f);
            tc.Rotation = glm::radians(rotation);
            DrawVec3Control("Scale", tc.Scale, 0.1f);
            ImGui::TreePop();
        }
    ...
    if (entity.HasComponent<CameraComponent>())
    {
        if (ImGui::TreeNodeEx((void *)typeid(CameraComponent).hash_code(), treeNodeFlags, "Camera"))
        {
            ...
            const char *projectionTypeStrings[] = {"Perspective", "Orthographics"};

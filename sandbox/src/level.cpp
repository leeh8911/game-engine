...
void Level::OnUpdate(gauri::Timestep ts)
{
...
void Level::OnRender()
{
    ...
    // Background
    gauri::Renderer2D::DrawQuad({playerPos.x, 0.0f, -0.8f}, {50.0f, 50.0f}, {0.3f, 0.3f, 0.3f, 1.0f});

    // Floor and ceiling
    gauri::Renderer2D::DrawQuad({playerPos.x, 34.0f}, {50.0f, 50.0f}, color);
    gauri::Renderer2D::DrawQuad({playerPos.x, -34.0f}, {50.0f, 50.0f}, color);
...
bool Level::CollisionTest()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        playerTransformedVerts[i] =
            ...
            glm::scale(glm::mat4(1.0f), {1.0f, 1.0f, 1.0f}) * playerVertices[i];
    }

    glm::vec4 pillarVertices[3] = {{-0.5f + 0.1f, -0.5f + 0.1f, 0.0f, 1.0f},
                                   {0.5f - 0.1f, -0.5 + 0.1f, 0.0f, 1.0f},

...
void Sandbox2D::OnImGuiRender()
{
    ...
    // - we allow the host window to be floating/moveable instead of filling the viewport (when opt_fullscreen ==
    // false)
    // - we allow the host window to have padding (when opt_padding == true)
    // - we have a local menu bar in the host window (vs. you could use BeginMainMenuBar() + DockSpaceOverViewport()
    // in your code!) TL;DR; this demo is more complicated than what you would normally use. If we removed all the
    // options we are showcasing, this demo would become:

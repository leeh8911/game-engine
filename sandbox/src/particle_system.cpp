...
void ParticleSystem::OnRender(gauri::OrthographicCamera &camera)
{
    gauri::Renderer2D::BeginScene(camera);
    for (auto &particle : m_ParticlePool)
    {
        if (!particle.Active)
            continue;
        ...
        gauri::Renderer2D::DrawQuad(position, {size, size}, glm::radians(particle.Rotation), color);
    }
